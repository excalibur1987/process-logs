<script lang="ts">
  ...
  onMount(fetchLogs);
  // Start polling for updates if the function is still running
  $effect(() => {
    if (pollingInterval) {
      clearInterval(pollingInterval);
    }

    // Check if any log has type "FINAL" to determine if we should stop polling
    const hasFinalLog = logs.some((log) => log.type === "FINAL");
    if (!hasFinalLog) {
      pollingInterval = setInterval(fetchLogs, 5000) as unknown as number;
    }
