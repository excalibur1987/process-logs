<script lang="ts">
	...
	let { funcId, initialLogs, showHeader = false, pollingInterval }: Props = $props();

	let logs = $state<FunctionLog[]>([]);
	let func = $state<FunctionInstance | null>(null);
...
{:else}
	<div
		class="max-h-[600px] space-y-4 overflow-y-auto pr-2"
								...
								<div class="mt-4">
									<div class="collapse collapse-plus bg-base-200">
										...
										<div class="collapse-content">
											<pre
												class="whitespace-pre-wrap font-mono text-sm text-error">{log.traceBack}</pre>
										</div>
									</div>
								</div>
							{/if}
